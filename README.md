...
系统通过 V4L2 采集工业相机视频流，利用 OpenCV 模板匹配算法实时追踪靶标位移，集成 **张正友相机标定** 实现像素-物理尺度自动转换，引入 **Kalman 滤波** 抑制模板匹配随机抖动，通过 **StatusAnalyzer** 实现在线质量评估与分级预警。在 Qt GUI 上同步显示视频画面与位移时序曲线，支持靶标管理、零点校准与数据导出。基础版本已在 RK3576 嵌入式平台（ARM64）完成运行验证，当前版本在此基础上增加相机标定、Kalman滤波及状态评估模块。
...
| 视觉算法 | OpenCV 4.x (模板匹配, 亚像素插值, `calib3d`, `Kalman Filter`) |
...
**注**：以上为系统早期版本运行截图，当前代码版本新增相机标定模块（CalibrationManager）与 Kalman 滤波，靶标信息面板将自动显示标定状态与精确像素尺度。
...
| `位移平滑` | 引入 Kalman 滤波（状态向量 `[x, y, vx, vy]`）抑制模板匹配随机抖动 |
