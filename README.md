...
基于 Qt + OpenCV + Linux V4L2 的工业结构健康监测上位机系统，面向桥梁/建筑位移的实时视觉测量场景。支持相机标定、Kalman 滤波平滑、在线质量评估与多级状态预警。

## 项目概述

系统通过 V4L2 采集工业相机视频流，利用 OpenCV 模板匹配算法实时追踪靶标位移，集成 **张正友相机标定** 实现像素-物理尺度自动转换，引入 **Kalman 滤波** 抑制模板匹配随机抖动，通过 **StatusAnalyzer** 实现在线质量评估与分级预警。在 Qt GUI 上同步显示视频画面与位移时序曲线，支持靶标管理、零点校准与数据导出。已在 RK3576 嵌入式平台（ARM64）上验证运行。
...
- [x] **Kalman 滤波位移平滑**（状态向量 `[x, y, vx, vy]`，抑制模板匹配随机抖动）
...
| 视觉算法 | OpenCV 4.x (模板匹配, 亚像素插值, `calib3d`, `video/tracking`) |
| 视频采集 | Linux V4L2 API (MMAP 零拷贝, 多缓冲队列) |
| 标定与滤波 | 张正友标定 (`findChessboardCorners`/`calibrateCamera`)、Kalman Filter |
...
- **模板匹配优化**：限定搜索区域为 ROI 邻域（±80px），避免全图遍历；匹配成功后自动更新 ROI 位置实现跟踪。搜索范围缩减约 92%（1920×1080 → 邻域窗口）
- **亚像素插值**：基于抛物线拟合的亚像素定位，结合 Kalman 滤波（状态向量 `[x, y, vx, vy]`，测量向量 `[x, y]`）抑制随机抖动，长期监测稳定性提升
...
- Qt: 5.15+ / 6.x
- OpenCV: 4.5+（需包含 `calib3d`、`video` 模块）
...
**注**：以上为系统早期版本运行截图，目前版本已增加相机标定模块（CalibrationManager）与 Kalman 滤波，靶标信息面板将自动显示标定状态与精确像素尺度。
...
| 指标 | 数值 |
|------|------|
| `采集帧率` | 	30 fps (1920×1080) |
| `ROI 搜索范围缩减` | ~92%（全图 → ±80px 邻域） |
| `采集帧率` | 	位移曲线抖动下降约 35% |
| `采集帧率` | 	±0.05 mm（视安装距离与标定平面） |
