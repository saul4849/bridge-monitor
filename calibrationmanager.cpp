...
CalibrationManager::CalibrationManager()
    : m_calibrated(false)
    , m_pixelScale(0.05)        // 默认回退系数，未标定时使用
    , m_focalLength(0.0)
{
...
bool CalibrationManager::loadFromFile(const QString& filepath)
{
    ...
    if (!fs["focal_length"].empty())
        fs["focal_length"] >> m_focalLength;
    else if (!m_cameraMatrix.empty() && m_cameraMatrix.at<double>(0, 0) > 0)
        m_focalLength = m_cameraMatrix.at<double>(0, 0);

    if (!fs["pixel_scale"].empty())
        fs["pixel_scale"] >> m_pixelScale;
    else if (m_focalLength > 0)
        m_pixelScale = 1100.0 / m_focalLength;  
    ...
    if (m_calibrated && m_imageSize.width > 0 && m_imageSize.height > 0
        && !m_cameraMatrix.empty() && !m_distCoeffs.empty()) {
    ...
    qDebug() << "[CalibrationManager] Loaded. Calibrated:" << m_calibrated
             << "PixelScale:" << m_pixelScale << "Focal:" << m_focalLength;
    return true;
}
...
bool CalibrationManager::calibrate(const std::vector<cv::Mat>& images, cv::Size boardSize, float squareSize)
{
    ...
    if (imagePoints.size() < 3) {
        qDebug() << "[CalibrationManager] Need at least 3 valid images, got:" << imagePoints.size();
        return false;
    }
    ...
    if (!tvecs.empty()) {
        double avgDist = 0.0;
        for (const auto& t : tvecs) avgDist += cv::norm(t);
        avgDist /= tvecs.size();

        m_focalLength = m_cameraMatrix.at<double>(0, 0);
        m_pixelScale = avgDist / m_focalLength;
    }
    ...
    qDebug() << "[CalibrationManager] Calibration done. RMS:" << rms
             << "AvgDist:" << (tvecs.empty() ? 0.0 : m_pixelScale * m_focalLength)
...
double CalibrationManager::pixelToMM(double pixelDistance, double targetDistanceMM) const
{
    if (!m_calibrated || m_focalLength <= 0) {
        return pixelDistance * m_pixelScale;   // 无标定文件时回退默认系数
    }
